...
export class ProductResolver {
  @Query(() => [Product])
  async product(@Args() { id, user, category }: ProductArgs): Promise<Product[]> {
    return new ProductService().list(id, user, category);
  }

  @Mutation(() => Product)
  async addProduct(@Arg('input') input: NewProduct): Promise<Product> {
  ...
  @Mutation(() => Product)
  async removeProduct(@Args() { id }: RemoveProductArgs): Promise<Product> {
